...
INTERFACE
    VAR
        ...
        vCrashFlagX : INT; (*If collission is detected, holds position of collission x axis *)
        vCrashFlagY : INT; (*If collission is detected, holds position of collission y axis *)
        Blinker : BLINK; (*Code that blinks a variable*)
        vFaultReason : None; (*If collission or dangerous combination detected, holds more information where it has occured*)
    END_VAR
END_INTERFACE
PROGRAM simulator:
    MOVE_CARS();
